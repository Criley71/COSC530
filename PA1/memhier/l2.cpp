...
L2_block::L2_block(int i, int t, int a, int db){
  index = i;
  tag = t;
...
L2::L2(int sc, int ss, int ls, bool wawb, int ibs, int obs){
  ...
  for(int i = 0; i < set_count; i++){
    l2_cache[i].resize(set_size, L2_block(-1, -1, -1, -1));
  }
}

void L2::insert_to_l2(int dec_l2_index, int dec_l2_tag, int time, int dirty_bit){
...
bool L2::check_l2(int dec_l2_index, int dec_l2_tag, int time, int dirty_bit){
  for(int i = 0; i < set_size; i++){
  ...
  insert_to_l2(dec_l2_index, dec_l2_tag, time, dirty_bit);
  return false;
}