  ...
  while (getline(cin, line)) {
    ...
    if (!config.dc_write_thru_no_allo && !config.l2_write_thru_no_allo) {
      if (!config.virt_addr) {
        ...
        if (!config.l2_enabled) {
          ...
          if (L2_CACHE.check_l2(l2_index, l2_tag, config.counter, dirty_bit, -1, false, dc_index, dc_tag)) {
            ...
            if (config.virt_addr) {
              ...
              insert_pfn = phys_page_num;
            }
            pair<bool, string> was_l2_replaced_and_if_yes_dc_phys_address = L2_CACHE.insert_to_l2(l2_index, l2_tag, config.counter, dirty_bit, insert_pfn, dc_index, dc_tag, bin_string, memory_refs);
        ...
        pfn = PAGE_TABLE.check_page_table(virt_page_num, config.counter, !is_read);
        // cout << " " << PAGE_TABLE.page_table[virt_page_num].valid << "<-isvalid";
        if (pfn == -1) {
          pfn = PAGE_TABLE.insert_page(virt_page_num, config.virtual_page_count, config.physical_page_count, config.counter, !is_read, DATA_CACHE, L2_CACHE, disk_refs, translation_buffer, memory_refs, l2_hits).first;
        ...
        int physical_address = PAGE_TABLE.vpn_to_phys_address(virt_page_num, page_off, config.pt_offset_bit, config.virtual_page_count, config.physical_page_count, config.counter, !is_read, DATA_CACHE, L2_CACHE, config.counter, translation_buffer, memory_refs, l2_hits);
        ...
        if (DATA_CACHE.check_cache(dc_index, dc_tag, config.counter, !is_read, pfn, false)) {
          ...
          pair<bool, string> was_dc_replaced_and_was_dirty = DATA_CACHE.insert_to_cache(dc_index, dc_tag, config.counter, dirty_bit, insert_pfn, p_bin_string);
          bool dirty_replaced = was_dc_replaced_and_was_dirty.first;
          string check_for_replace = was_dc_replaced_and_was_dirty.second;
          dc_misses += 1;
          if (config.l2_enabled) {
        ...
        if (!config.l2_enabled) {
          ...
          if (L2_CACHE.check_l2(l2_index, l2_tag, config.counter, dirty_bit, pfn, false, dc_index, dc_tag)) {
            ...
            pair<bool, string> was_l2_replaced_if_yes_dc_phys_address = L2_CACHE.insert_to_l2(l2_index, l2_tag, config.counter, dirty_bit, insert_pfn, dc_index, dc_tag, p_bin_string, memory_refs);
      ...
      if (!config.virt_addr) {
        ...
        phys_page_num = stoi(phys_page_num_bin, 0, 2);
        if (memory_refs == 0) {
          memory_refs += 2; // i am consistantly 2 refs short when this config is enabled
        }
        if (phys_page_num >= config.physical_page_count) {
          cout << "hierarchy: physical address " << hex << temp << " is too large.\n";
          exit(-1);
        ...
        if (!config.l2_enabled) {
          ...
          if (L2_CACHE.check_l2(l2_index, l2_tag, config.counter, dirty_bit, -1, false, dc_index, dc_tag)) {
            ...
            if (was_there_an_l2_eviction) {
              ...
              bool was_dc_dirty = DATA_CACHE.evict_given_l2_phys_address(dc_index, dc_tag);
              if (was_dc_dirty && temp == memory_refs) {
                // memory_refs += 1;
                t5 += 1;
              }
              t3 += 1;
        ...
        if (DATA_CACHE.check_cache(dc_index, dc_tag, config.counter, !is_read, -1, false)) {
          ...
          if (is_read) {
            ...
            DATA_CACHE.insert_to_cache(dc_index, dc_tag, config.counter, dirty_bit, insert_pfn, phys_page_num_bin);
          }
          dc_misses += 1;
          if (config.l2_enabled) {
            cout << " miss";
        ...
        if (!config.l2_enabled) {
          ...
          if (L2_CACHE.check_l2(l2_index, l2_tag, config.counter, dirty_bit, pfn, false, dc_index, dc_tag)) {
            ...
            pair<bool, string> was_l2_replaced_and_if_yes_dc_phys_address = L2_CACHE.insert_to_l2(l2_index, l2_tag, config.counter, dirty_bit, insert_pfn, dc_index, dc_tag, p_bin_string, memory_refs);
            // the insert to l2 function will fully determine if mem ref incrememnt as it checks in the function for the dirty bit
            bool was_there_an_l2_eviction = was_l2_replaced_and_if_yes_dc_phys_address.first;
            if (was_there_an_l2_eviction) {
              string replace_address = was_l2_replaced_and_if_yes_dc_phys_address.second;
              ...
              bool check = DATA_CACHE.evict_given_l2_phys_address(dc_index, dc_tag);
              if(check){
                memory_refs+=1;
              }
              // dc never is dirty as any write writes to l2
        ...
        if (DATA_CACHE.check_cache(dc_index, dc_tag, config.counter, !is_read, -1, false)) {
          ...
          if (is_read) {
            ...
            pair<bool, string> checker = DATA_CACHE.insert_to_cache(dc_index, dc_tag, config.counter, dirty_bit, insert_pfn, phys_page_num_bin);
            if(checker.first){
              //memory_refs += 1;
            }
          }
      ...
      if (!config.virt_addr) {
        ...
        if (DATA_CACHE.check_cache(dc_index, dc_tag, config.counter, !is_read, -1, false)) {
          ...
          pair<bool, string> was_dc_replaced_and_was_dirty = DATA_CACHE.insert_to_cache(dc_index, dc_tag, config.counter, dirty_bit, insert_pfn, bin_string);
          bool dirty_replaced = was_dc_replaced_and_was_dirty.first;
          string check_for_replace = was_dc_replaced_and_was_dirty.second;
        ...
        if (!config.l2_enabled) {
          ...
          if (L2_CACHE.check_l2(l2_index, l2_tag, config.counter, dirty_bit, pfn, false, dc_index, dc_tag)) {
            ...
            t6 += 1;
            // memory_refs += 1; //could also be this increment
            cout << " miss\n";
          }
        }
      } else if (config.virt_addr && config.dtlb_enabled) {
        ...
        cout << setw(4) << setfill(' ') << hex << tlb_index;
        if (translation_buffer.check_dtlb(tlb_index, tlb_tag, config.counter)) {
          // cout << "tlb index ->"<<tlb_index << " tlb tag ->" << tlb_tag <<" IS A FUCKING HIT SOMEHOW |";
          // translation_buffer.insert_to_dtlb(tlb_index, tlb_tag, config.counter, pfn);
          cout << setw(4) << setfill(' ') << " hit ";
          cout << setw(5) << setfill(' ') << " ";
          dtlb_hits += 1;
          ...
          cout << setw(4) << setfill(' ') << " miss";
          dtlb_misses += 1;
          // pfn = PAGE_TABLE.get_current_page_count();
        ...
        if (DATA_CACHE.check_cache(dc_index, dc_tag, config.counter, !is_read, pfn, false)) {
          ...
          pair<bool, string> was_dc_replaced_and_was_dirty = DATA_CACHE.insert_to_cache(dc_index, dc_tag, config.counter, dirty_bit, insert_pfn, p_bin_string);
          bool dirty_replaced = was_dc_replaced_and_was_dirty.first;
          if(dirty_replaced && !config.l2_enabled){
            memory_refs+=1;
          }
          string check_for_replace = was_dc_replaced_and_was_dirty.second;
          dc_misses += 1;
      ...
      if (!config.virt_addr) {
        ...
        if (DATA_CACHE.check_cache(dc_index, dc_tag, config.counter, !is_read, pfn, false)) {
          ...
          if (is_read) {
            ...
            // memory_refs += 1; IT COULD ALSO BE THIS MEMORY REF INCREMENTER
            t4 += 1;
          }
        } else {
          if (is_read) {
      ...
      } else if (config.dtlb_enabled && config.virt_addr) {
        virt_page_num_bin = bin_string.substr(0, 64 - config.pt_offset_bit);
        virt_page_num = stoi(virt_page_num_bin, 0, 2);
        if (virt_page_num >= config.virtual_page_count) {
          cout << "hierarchy: virtual address " << hex << temp << " is too large.\n";
        ...
        // DC INDEX FOR VIRTUAL ADDRESSES
        dc_index_bin = p_bin_string.substr((64 - (config.dc_index_bits + config.dc_offset_bits)), config.dc_index_bits);
        // cout << "\ndc_index_bin: " << dc_index_bin << "\n";
        // cout << "p_bin_string: " << p_bin_string << "\n";
        if (config.dc_index_bits == 0) {
          dc_index = 0;
        } else {
          dc_index = stoi(dc_index_bin, 0, 2);
