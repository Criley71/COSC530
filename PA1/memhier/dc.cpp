...
Cache_block::Cache_block(int i, int t, int a, int db, int pf, string pa) {
  index = i;
  tag = t;
...
pair<bool, string> DC::insert_to_cache(int dc_index, int dc_tag, int time, int dirty_bit, int pfn, string phys_address) {
  ...
  for (int i = 0; i < set_size; i++) {
    if (data_cache[dc_index][i].time_last_accessed < oldest_used) {
      ...
      old_addr = data_cache[dc_index][i].phys_addr;
    }
  }
...
bool DC::evict_given_l2_phys_address(int dc_index, int dc_tag){
